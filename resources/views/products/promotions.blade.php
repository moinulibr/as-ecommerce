...
<div class="container mx-auto md:py-6">
    <div class="flex flex-col md:flex-row">
        ...
        <div class="md:w-9/12 p-4 md:pt-1 md:pl-8 md:pr-0">
            <h1 class="text-xl font-bold mb-4">{{ __('messages.all_promotion_products') }}</h1>
            <!-- Search Bar -->
...
<script>
    ...
    function fetchData(page=null){
        ...
        var category_id=category.get();

      var subcategory = $('.subcategory:checked').map(function(){
         return $(this).val();
      });
      var sub_category_id=subcategory.get();
    
      var brand_id=$('.brands').val();

    
      var q=$('input.search2').val();
      var shorting=$('select.shorting').val();
      let max_price=$('.noUi-handle').attr('aria-valuemax');
      let min_price=$('.noUi-handle').attr('aria-valuenow');
      $.ajax({
         type:'GET',
         url:'{{ route("front.products.promotions")}}?page='+page,
         data:{max_price,min_price,sub_category_id,q,shorting,brand_id, category_id},

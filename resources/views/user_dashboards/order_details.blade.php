    ...
    <div class="container mx-auto p-4 max-w-4xl">
        ...
        <section class="bg-white shadow rounded-lg p-6">
            ...
            <div class="space-y-4">
                ...
                @foreach($item->vendor_orders as $vendorOrder)
                <!-- Vendor Header -->
                <div class="bg-gray-100 p-3 rounded-md mb-4">
                    <h3 class="text-lg font-semibold text-gray-800">
                        Vendor: {{ $vendorOrder->user->name ?? 'N/A' }}, Shipping Status: {{ $vendorOrder->shipping_status }}, Payment Status: {{ $vendorOrder->payment_status }}
                    </h3>
                </div>
                ...
                @foreach($vendorOrder->lines as $line)
                    <div class="flex justify-between items-center border-b pb-4 mb-2">
                        <div class="flex items-center">
                            <img src="{{ $line->product->image_url }}" class="w-20 h-20 object-cover rounded mr-4">
                            <div>
                                <h4 class="text-md font-medium text-gray-800">
                                    {{ $line->product->name }}
                                    {{ $line->product->type == 'variable' ? $line->variation->name : '' }}
                                </h4>
                                <p class="text-gray-600">Qty: {{ $line->quantity }}</p>
                                <p class="text-gray-600">Price: {{ priceFormate($line->price) }}</p>
                            </div>
                        </div>
            
                        <p class="text-gray-800 font-medium">
                            {{ priceFormate($line->price * $line->quantity) }}
                        </p>
                    </div>
                @endforeach
                ...
                <div class="text-right font-semibold text-gray-700 mt-2">
                    Subtotal: {{ priceFormate($vendorOrder->final_amount) }}
                </div>
